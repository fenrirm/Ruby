...
class MailContainer
  ...
  def send_letter(to, initial_date, content)
    outgoing.push(Outgoing.new("Marta Shevchenko", to, initial_date, content))
  end
  ...
  def find_by_title(query)
    print("Found:\n", incoming.select { |a| a.content.header.match("/^"+query+"/") }).to_s
  end
...
mail.receive_letter(Incoming.new("Damon Salvatore", "Me", Date.new(2022, 3, 12), Content.new("Header", "Text"), false))
mail.receive_letter(Incoming.new("Elena Gilbert", "Me", Date.new(2022, 5, 12), Content.new("Header2", "Text2"), false))
mail.receive_letter(Incoming.new("Caroline Forbes", "Me", Date.new(2022, 5, 12), Content.new("Header2", "Text2"), true))
mail.send_letter("Rachel Green", Date.new(2020, 5, 16), Content.new("Header3", "Text3"))
